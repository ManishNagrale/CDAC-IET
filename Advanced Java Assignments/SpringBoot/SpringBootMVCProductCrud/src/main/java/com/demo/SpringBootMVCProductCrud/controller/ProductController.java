...
public class ProductController {
	...
	public ModelAndView getAllProducts() {
		List<Product2> plist=pservice.findAllProducts();
	...
	public String showaddproductform(Model m) {
		m.addAttribute("product",new Product2());
		return "insertProduct";
		
	}
	
	@PostMapping("/insertProduct")
	public ModelAndView insertProduct( @ModelAttribute Product2 product) {
	...
	public ModelAndView editProduct(@PathVariable int pid) {
		Product2 p=pservice.getById(pid);
		if(p!=null) {
	...
	public ModelAndView updateProduct(@ModelAttribute Product2 prod) {
